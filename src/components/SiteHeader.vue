  ...
  >
    ...
    <div class="section flex items-center justify-between h-14">
      ...
      <div class="hidden md:flex items-center gap-2">
        <button v-if="!isAuthed" class="nav-cta" @click.stop.prevent="onSignin">Sign in</button>
        ...
        <div v-else class="relative usermenu-root">
          ...
          <div v-if="userMenu" class="user-menu">
            <RouterLink class="user-item" to="/account" @click="userMenu = false">
              <span class="ico"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm7 8a7 7 0 0 0-14 0" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></span>
              <span>Account</span>
            </RouterLink>
  ...
  <span class="flex items-center gap-2">
    ...
    <span class="badge soon">Soon</span>
            ...
            <RouterLink class="user-item" to="/messages" @click="userMenu = false">
              <span class="ico"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M21 12a7 7 0 0 1-7 7H7l-4 4V5a7 7 0 0 1 7-7h4a7 7 0 0 1 7 7Z" transform="translate(0 3)" stroke="currentColor" stroke-width="1.5"/></svg></span>
              <span class="flex items-center gap-2">Messages <span v-if="unreadCount > 0" class="badge ok">{{ unreadCount }}</span></span>
            </RouterLink>
            ...
            <button class="user-item danger" @click.stop.prevent="onLogout">
              <span class="ico"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M9 21H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h3" stroke="currentColor" stroke-width="1.5"/><path d="M16 17l5-5-5-5M21 12H9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></span>
              <span>Log out</span>
            </button>
          </div>
        </div>
      ...
      <button class="md:hidden burger" @click="open = !open" :aria-expanded="open">
        ...
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
          ...
          <path :class="{ 'opacity-100': open, 'opacity-0': !open }" d="M6 6l12 12M18 6L6 18" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
        </svg>
      </button>
    </div>
...
<RouterLink v-if="isAuthed" class="drawer-link" :to="profileUrl" @click="open = false">Profile</RouterLink>
          ...
          <div class="mt-3 grid grid-cols-2 gap-2">
            ...
            <button v-else type="button" class="col-span-2 nav-cta" @click.stop.prevent="$emit('logout'); open = false;">Log out</button>
          </div>
        </div>
      </div>
    </transition>
...
watch(() => user.value?.id, () => loadHeaderProfile(), { immediate: true })

/* label + буква-инициал */
const displayLabel = computed(() => {
  const metaName =
    (user.value?.user_metadata?.username ||
     user.value?.user_metadata?.desired_username || '') as string
  const name = profileUsername.value || metaName || (email.value ? email.value.split('@')[0] : 'Account')
  return name
})
...
const profileUrl = computed(() =>
  (profileUsername.value || (user.value?.user_metadata?.desired_username as string) || '')
    ? `/u/${encodeURIComponent(profileUsername.value || (user.value?.user_metadata?.desired_username as string))}`
    : '/profile'
)
...
function onProfileUpdated(e: Event) {
  ...
  if ('username' in d) {
    profileUsername.value = d.username || null
  }
  if ('avatar_url' in d) {
...
.avatar-img{
  width:22px; height:22px; border-radius:9999px; object-fit:cover;
  border:1px solid rgba(255,255,255,.18);
}
